...
def run_traditional_full(n_personas=8, em_iters=100, l1_lambda=1.0, subset_chars=None, 
                         data_file=None, word_csv=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f">>> Traditional SAGE (Cluster-based): P={n_personas}, iters={em_iters}, l1={l1_lambda}")
    ...
    # 1. Prepare Semantic Tree (if needed)
    df_c = pd.read_csv(word_csv)
    if 'vector' not in df_c.columns:
        print(">>> Computing BERT PCA-100 centroids for semantic tree...")
        df_vocab = pd.read_csv("data/processed/female_vocab_map.csv")
        df_emb = pd.read_csv("data/processed/female_bert_pca100_embedding.csv")
        df_emb['vec_arr'] = df_emb['bert_pca_100'].apply(lambda x: np.array([float(v) for v in x.split(',')]))
        ...
        word_csv = "data/processed/temp_cluster_vectors.csv"
    ...
    res_dir = os.path.abspath(f"data/results/traditional_results/P{n_personas}_L{l1_lambda}")
    ckpt_dir = os.path.abspath(f"checkpoints/traditional_P{n_personas}_L{l1_lambda}")
    ...
    print(f">>> Assignments saved to {res_dir}")

    # 4. Evaluate Metrics
    print(">>> Evaluating Metrics...")
    # For traditional SAGE, use distribution-based silhouette
    ...
    # 5. Extract Keywords
    print(">>> Extracting Top Words per Persona...")
    all_word_weights = []
    with torch.no_grad():
        V_total = model.word_paths.shape[0]
    ...
    pd.DataFrame(all_word_weights).to_csv(kw_path, index=False)
...
if __name__ == "__main__":
    ...
    run_traditional_full(n_personas=args.n_personas, em_iters=args.iters, l1_lambda=args.l1, 
                         subset_chars=args.subset, data_file=args.data_file, word_csv=args.word_csv)
