...
def run_female_top300():
    ...
    out_dir = 'data/results/female_top300'
    os.makedirs(out_dir, exist_ok=True)
    
    print(f">>> Initializing CVAE-SAGE for Top 300 Female Characters (iters={iters})")
    ...
    # Filter female characters and then top 300 by word frequency
    female_keys = females['char_key'].tolist()
    female_df = df_full[df_full['char_key'].isin(female_keys)]
    word_freq = female_df.groupby('char_key')['count'].sum()
    top300_keys = word_freq.sort_values(ascending=False).head(300).index.tolist()
    df_full = female_df[female_df['char_key'].isin(top300_keys)].copy()
    
    print(f"    Total characters loaded (top 300): {len(df_full['char_key'].unique())}")
